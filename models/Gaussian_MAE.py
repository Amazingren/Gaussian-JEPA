...
def chamfer_distance(x, y, norm=2):
    # x, y: (B, N, D)
    diff = x.unsqueeze(2) - y.unsqueeze(1)  # (B, N, M, D)
    dist = (diff ** 2).sum(-1)  # (B, N, M) — squared L2
    if norm != 2:
        dist = dist.sqrt()
    loss = dist.min(2)[0].mean() + dist.min(1)[0].mean()
    return loss, None
...
class Gaussian_MAE(nn.Module):
    def __init__(self, config):
        ...
        # Full PLY export requires every 3DGS channel. E(C) remains a valid
        # xyz reconstruction model but skips final-epoch Gaussian export.
        full_gs_attributes = {"xyz", "opacity", "scale", "rotation", "sh"}
        self.supports_ply_save = full_gs_attributes.issubset(self.attribute)
...
@MODELS.register_module()
class Gaussian_MAE_EC(Gaussian_MAE):
    """Centroid-only Gaussian-MAE baseline for the matched E(C) experiment.

    The explicit registry entry prevents an E(All) config from being mistaken
    for E(C). The inherited model reconstructs only masked xyz neighborhoods
    and therefore returns only the Chamfer-distance objective.
    """

    def __init__(self, config):
        if list(config.attribute) != ["xyz"]:
            raise ValueError(f"Gaussian_MAE_EC requires attribute=['xyz'], got {config.attribute}")
        if list(config.group_attribute) != ["xyz"]:
            raise ValueError(
                f"Gaussian_MAE_EC requires group_attribute=['xyz'], got {config.group_attribute}"
            )
        super().__init__(config)
        self.supports_ply_save = False
        print_log("[Gaussian_MAE_EC] xyz-only reconstruction; PLY export disabled", logger="Gaussian_MAE_EC")
...
@MODELS.register_module()
class PointTransformer_MAE_EC(PointTransformer):
    """Strict xyz-only downstream classifier for Gaussian-MAE E(C)."""

    def __init__(self, config, **kwargs):
        if list(config.attribute) != ["xyz"]:
            raise ValueError(
                f"PointTransformer_MAE_EC requires attribute=['xyz'], got {config.attribute}"
            )
        if list(config.group_attribute) != ["xyz"]:
            raise ValueError(
                "PointTransformer_MAE_EC requires group_attribute=['xyz'], "
                f"got {config.group_attribute}"
            )
        super().__init__(config, **kwargs)

    def load_model_from_ckpt(self, ckpt_path):
        if ckpt_path is None:
            print_log("Training from scratch!!!", logger="PointTransformer_MAE_EC")
            self.apply(self._init_weights)
            return

        ckpt = torch.load(ckpt_path, map_location="cpu")
        if "base_model" not in ckpt:
            raise KeyError(f"Checkpoint has no 'base_model': {ckpt_path}")
        base_ckpt = {
            key.replace("module.", "", 1): value
            for key, value in ckpt["base_model"].items()
        }
        remapped = {
            key[len("MAE_encoder.") :]: value
            for key, value in base_ckpt.items()
            if key.startswith("MAE_encoder.")
        }
        if not remapped:
            raise RuntimeError(
                f"No MAE_encoder.* weights found in E(C) checkpoint: {ckpt_path}"
            )

        incompatible = self.load_state_dict(remapped, strict=False)
        core_prefixes = ("encoder.", "pos_embed.", "blocks.", "norm.")
        missing_core = [
            key for key in incompatible.missing_keys if key.startswith(core_prefixes)
        ]
        if missing_core:
            raise RuntimeError(
                "Incomplete MAE E(C) encoder load:\n"
                + get_missing_parameters_message(missing_core)
            )
        if incompatible.missing_keys:
            print_log(
                get_missing_parameters_message(incompatible.missing_keys),
                logger="PointTransformer_MAE_EC",
            )
        if incompatible.unexpected_keys:
            print_log(
                get_unexpected_parameters_message(incompatible.unexpected_keys),
        ...
        print_log(
            f"[PointTransformer_MAE_EC] Loaded encoder from {ckpt_path}",
            logger="PointTransformer_MAE_EC",
        )
