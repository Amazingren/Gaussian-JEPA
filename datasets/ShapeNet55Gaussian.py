...
class ShapeNetGaussian(data.Dataset):
    def __init__(self, config):
        self.data_root = os.environ.get("SHAPENET55GS_SPLIT_ROOT") or config.DATA_PATH
        self.gs_path = os.environ.get("SHAPENET55GS_PLY_ROOT") or config.GS_PATH
        print_log(f"[DATASET] split root: {self.data_root}; PLY root: {self.gs_path}", logger="ShapeNetGS-55")
    ...
    def __getitem__(self, idx):
        ...
        file_path = os.path.join(self.gs_path, sample["file_path"])
        try:
            gs = IO.get(file_path)
