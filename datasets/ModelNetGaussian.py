...
class ModelNetGaussian(data.Dataset):
    def __init__(self, config):
        self.data_root = os.environ.get("MODELNETGS_SPLIT_ROOT") or config.DATA_PATH
        self.gs_path = os.environ.get("MODELNETGS_PLY_ROOT") or config.GS_PATH
        self.num_category = config.NUM_CATEGORY
        self.attribute = config.ATTRIBUTE
        self.subset = config.subset
        self.norm_attribute = config.norm_attribute
        print_log(
            f"[DATASET] split root: {self.data_root}; PLY root: {self.gs_path}",
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load {sample['file_path']}") from exc
