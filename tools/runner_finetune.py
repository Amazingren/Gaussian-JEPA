...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            # Clamp the legacy sampling pool when a dataset provides exactly
            # the requested number of input primitives.
            point_all = min(point_all, points.size(1))
            if point_all < npoints:
                raise ValueError(
                    f"Input contains {points.size(1)} points, fewer than npoints={npoints}"
                )
